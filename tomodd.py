...
# -*- coding: utf-8 -*-
"""
将地震台网 DBO/DPB 或旧 JOPENS 观测报告转换为 hypoDD/tomoDD phase.dat。

本版本不再依赖 idp_hypoDD.inp 和 catalog.txt。输入文件、输出文件与
起始事件编号既可以在“用户配置区”直接修改，也可以通过命令行覆盖。
...
4. PmP、SmS、SME、SMN 等反射相/振幅记录不作为 P/S 首波写出。
5. 若同一台站最早 S 不晚于最早 P，则寻找下一条晚于 P 的有效 S；
   若不存在，则删除该异常 S，并在日志中记录。
6. 原始报告时间按北京时间（UTC+8）解释；输出事件头转换为 UTC；
   走时由同一时区下的到时减发震时刻得到。
7. 同时兼容：
   - 旧 DBO：YYYY-MM-DD，震级写作“ML 1.7”等；
   - 新 DBO：YYYY/MM/DD，震级直接位于深度之后；
   - DPB 日期同样兼容 '-' 与 '/'，并允许通道与震相之间出现 U/R 等极性字段；
   - 旧 JOPENS 固定列格式。
...
def write_event_block(handle, event: Event, event_id: int,
    ...
    utc_origin = event.origin_local.astimezone(UTC_TZ)
    utc_second = utc_origin.second + utc_origin.microsecond / 1_000_000.0

    handle.write(
        f"#{utc_origin.year:5d} {utc_origin.month:2d} {utc_origin.day:2d} "
        f"{utc_origin.hour:2d} {utc_origin.minute:2d} {utc_second:5.2f} "
...
def write_conversion_log(log_path: Path, cfg: Config,
    ...
    with log_path.open("w", encoding="utf-8") as handle:
        ...
        handle.write("===================================\n")
        handle.write("Time basis: input = China Standard Time (UTC+8); event output = UTC\n")
